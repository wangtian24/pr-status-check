...
_cache = {"my_prs_html": "", "fetched_epoch": 0}
...
def fetch_prs_for_repo(owner, name):
    ...
    fields = "number,title,isDraft,createdAt,updatedAt,reviewDecision,url,reviews,statusCheckRollup,mergeable,mergeStateStatus"
    ...
    open_prs = json.loads(result.stdout) if result.stdout.strip() else []
    for pr in open_prs:
...
def render_own_prs_section(owner, name, active, drafts):
...
def render_review_repo_section(owner, name, review_prs):
    if not review_prs:
        return ""
    html = f'<div class="repo-section"><h2 class="repo-name">{html_mod.escape(name)}</h2>'
    html += render_review_grouped(review_prs)
    html += "</div>"
    return html
...
def build_my_prs_body():
    ...
    for owner, name in CONFIG["repos"]:
        ...
        if result[0] == "error":
            ...
            sections += render_own_prs_section(owner, name, active, drafts)

    return sections or '<p class="empty">No open PRs found.</p>'
...
def build_review_body():
    ...
    for owner, name in CONFIG["repos"]:
        ...
        if result[0] == "error":
            ...
            sections += render_review_repo_section(owner, name, review_prs)
...
<style>
    ...
    .review-author {{ font-size: 15px; font-weight: 600; color: #57606a; margin: 16px 0 4px 0;
                      padding-left: 4px; border-bottom: 1px solid #e1e4e8; padding-bottom: 4px; }}
    .review-author-prs {{ margin-left: 8px; }}
    /* Tab bar */
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if not my_prs_html:
            ...
            page = build_full_page(my_prs_html, epoch)
        self.send_response(200)
        self.send_header("Content-Type", "text/html; charset=utf-8")
        self.end_headers()
